...
contract SwapAlgebra is ISwap {
    ...
    constructor(
        ...
        require(
            _algebraFactory != address(0),
            "Invalid Algebra factory address"
        );
        require(
    ...
    function setIntermediaryToken(
        ...
        require(
            ...
            "Invalid intermediary token address"
        );
    ...
    function swap(
        ...
        if (gasFee > 0) {
            ...
            if (tokenIn != gasZRC20) {
                ...
                uint256 amountInForGas = getAmountInForGasFee(
                    tokenIn,
                    gasZRC20,
                    gasFee
                );
                require(
                    amountInForGas < amountIn,
                    "Amount insufficient to cover gas fees"
                );
        ...
        if (amountInForMainSwap > 0) {
            ...
            if (directPoolExists) {
                ...
                require(
                    intermediaryToken != address(0),
                    "No intermediary token set for this token name"
                );

                // Check if both pools exist
                bool poolInToInterExists = algebraPoolExists(
                    tokenIn,
                    intermediaryToken
                );
                bool poolInterToOutExists = algebraPoolExists(
                    intermediaryToken,
                    tokenOut
                );

                require(
                    poolInToInterExists && poolInterToOutExists,
                    "Required Algebra pools do not exist"
                );
     ...
     * @return amountOut The amount of output tokens received
     */
    function swapThroughAlgebraPool(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
        ...
        uint256 currentAllowance = IERC20(tokenIn).allowance(
            address(this),
            pool
        );
